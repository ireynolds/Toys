...
//    int pageSize;
    ...
    */
    ...
    public synchronized void logCommit(long tid) throws IOException {
        preAppend();
        Debug.log("COMMIT " + tid);
        //should we verify that this is a live transaction?
    ...
    */
    public  synchronized void logWrite(TransactionId tid, Page before, Page after) throws IOException  {
        logWrite(tid.getId(), before, after);
    }
    
    public  synchronized void logWrite(long tid, Page before, Page after) throws IOException  {
    ...
    */
    public synchronized  void logXactionBegin(TransactionId tid) throws IOException {
        logXactionBegin(tid.getId());
    }
    
    public synchronized  void logXactionBegin(long tid) throws IOException {
            Debug.log("BEGIN");
    ...
    */
    ...
    public void rollback(long tid) throws NoSuchElementException, IOException {
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                ...
                // logAbort calls preAppend, which updates the totalRecords 
                // counter. Then logAbort calls rollback, which here calls 
                // preAppend again. Doesn't that double-count the abort record,
                // considering that only one record will be written for an
                // abort? Leaving this line in because it was given to us.
                preAppend();
                
                // A set of pages that have been restored to their pre-image.
                // For this forward-undo pass, you should only restore a page
                // to its pre-image the first time you see it.
                Set<PageId> pidsReset = new HashSet<PageId>();
                
                // Start at the begin statement for this transaction and 
                // continue until the end of the log. Only at the end of the
                // log can you be sure that you have reviewed every record
                // related to this transaction.
                
                long currRecOffset = tidToFirstLogRecord.get(tid);
                raf.seek(currRecOffset);
                
                while (raf.getFilePointer() != raf.length()) {
                    // What kind of record is this, and to which transaction 
                    // does it apply?
                    int recType = raf.readInt();
                    long recTid = raf.readLong();
                    
                    // "These are not the records you're looking for ..."
                    if (recTid != tid || recType != UPDATE_RECORD) {
                        skipToNextRecord(recType);
                        continue;
                    }
                    
                    // Get the preImage out of this record, and skip to the next
                    // record
                    Page preImage = readPageData(raf);
                    readPageData(raf);
                    raf.seek(raf.getFilePointer() + LONG_SIZE);
                    
                    // Restore the old version of the page if necessary
                    PageId preImageId = preImage.getId();                   
                    if (!pidsReset.contains(preImageId)) {
                        DbFile file = cat.getDbFile(preImageId.getTableId());
                        file.writePage(preImage);
                        pidsReset.add(preImageId);
                    }
                }
    ...
    // Assumes that the current position of raf points immediately after the
    // long that represents the tid. Returns the offset of the record it just
    // skipped.
    private long skipToNextRecord(int type) throws IOException {        
        switch (type) {
            case UPDATE_RECORD:
                readPageData(raf);
                readPageData(raf);
            ...
            case CHECKPOINT_RECORD:
                ...
                int sizeTransaction = 2 * LONG_SIZE;
                raf.seek(raf.getFilePointer() + numTransactions * sizeTransaction);
                break;
        }
        return raf.readLong();
    }
    ...
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                // ANALYSIS PASS: Build one set of transactions to redo (that 
                // explicitly committed), and one of transactions to undo (that
                // neither committed nor aborted). Do nothing with transactions
                // that explicitly aborted.
                Set<Long> redo = new HashSet<Long>();
                Set<Long> undo = new HashSet<Long>();
                
                if (ckptOffset != -1) {
                    raf.seek(ckptOffset + INT_SIZE + LONG_SIZE);
                    int numTransactions = raf.readInt();
                ...
                while (raf.getFilePointer() < raf.length()) {
                    ...
                    long currRecOffset = skipToNextRecord(type);
                    
                    // Build up the proper sets
                    switch (type) {
                        case BEGIN_RECORD:
                            tidToFirstLogRecord.put(tid, currRecOffset);
                            undo.add(tid);
                            break;
                        case COMMIT_RECORD:
                            redo.add(tid);
                ...
                // REDO PASS: Redo all transactions that need to be redone.
                if (ckptOffset != -1) {
                    raf.seek(ckptOffset);
                }
                ...
                while (raf.getFilePointer() < raf.length()) {
                    ...
                    // Redo if necessary
                    if (type == UPDATE_RECORD && redo.contains(tid)) {
                        readPageData(raf);
                        Page postImage = readPageData(raf);
                        raf.readLong();
                        
                        cat.getDbFile(postImage.getId().getTableId()).writePage(postImage);
                        buf.discardPage(postImage.getId());
                    } else {
                        skipToNextRecord(type);
                    }
                }
                
                // UNDO PASS: Undo all the transactions that need to be undone.             
                for (long tid : undo) {
                    logAbort(tid);
                }
    ...
    public void print() throws IOException {
        ...
        while (raf.getFilePointer() != raf.length()) {
            ...
            case COMMIT_RECORD:
                ...
                break;
            case UPDATE_RECORD:
                PageId pid = readPageData(raf).getId();
                readPageData(raf);
                System.out.println("UPDATE " + tid + ": table " + pid.getTableId() + ", page " + pid.pageNumber());
                break;
            case BEGIN_RECORD:
